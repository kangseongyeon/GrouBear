...
<form action="/employee/elaprInsert.do" id="formInsert" method="post">
	<c:if test="${status eq 'u' }">
		                                            ...
		                                            <div style="width: 100%; display: flex; justify-content: space-between;">
		                                               <div style="display: flex; align-items: center; width: 120%;">
			                                               	...
			                                               	<c:if test="${status eq 'i' }">
			                                               		<h2><font style="vertical-align: inherit;" class="me-4"><font style="vertical-align: inherit;">결재하기</font></font></h2>
			                                               	</c:if>
			                                               	<c:if test="${status eq 'u' }">
		                                    ...
		                                    <div class="card-body pt-0" style="margin-top: 20px;">
		                                    	<div class="mb-10 fv-row fv-plugins-icon-container">
		                                    		<div style="display: flex;">
								         			...
								         			<div class="card-toolbar d-flex justify-content-end md-3 my-3" data-kt-filemanager-table-toolbar="base">
													    <button type="button" id="cancleBtn" class="btn btn-flex btn-secondary me-2 px-4">
													        <i class="bi bi-x-circle">취소</i>
													    </button>
													    <button type="button" id="previewBtn2" class="btn btn-flex btn-light-primary me-2 px-4">
													        <i class="bi bi-eye">미리보기</i>
													    </button>
													    <c:if test="${status eq 'u' }">
													        <button type="button" id="delBtn" class="btn btn-flex btn-light-danger me-2 px-4">
													            <i class="bi bi-trash">삭제</i>
													        </button>
													    </c:if>
													    <button type="button" class="btn btn-flex btn-light-info me-1 px-4" data-bs-toggle="modal" data-bs-target="#reqElapr">
													        <i class="bi bi-pencil-square">결재요청</i>
													    </button>
													</div>
											  	</div>
											</div>
...
$(function () {
	...
	cancleBtn.on('click', function () {
		Swal.fire({
			...
			if (result.isConfirmed) {
				location.href = "/employee/empElaprDraft.do";		// 기안 문서함 이동
			}
		})
	});
	...
	addBtn.on('click', function () {
	    ...
	    // 수정일 때
	    if ($(this).val() == '결재재요청') {
	        formInsert.attr("action", "/employee/elaprUpdate.do");
	    }
	...
	delBtn.on('click', function () {
		Swal.fire({
			...
			if (result.isConfirmed) {
				formInsert.attr("action", "/employee/elaprDelete.do");
				formInsert.submit();
			}
		})
	});
	...
	$('#formCallJstree').on('click', function () {
		...
		$.ajax({
			type: "get",
			url: "/employee/elaprSelectJstree.do",
	...
	$("#previewBtn").on("click", function () {
		...
		$.ajax({
			url : "/employee/elaprSelectForm.do?docFormNo=" + docFormNoSelect,
			type : "get",
			success : function (data) {
				...
				$.ajax({
					url : "/employee/elaprSelectImpl.do",
					type : "get",
					dataType : "JSON",
					success : function (data) {
						console.log(data);
